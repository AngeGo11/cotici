...
import { buildSolidarityCollectLink } from '@/shared/api/solidarityApi';

function formatFcfa(n: number): string {
  return `${n.toLocaleString('fr-FR')} FCFA`;
}

export default function ShareSolidarityCollectScreen() {
  const router = useRouter();
  const params = useLocalSearchParams<{ id?: string; motif?: string; objectif?: string }>();

  const collectId = typeof params.id === 'string' ? params.id : '';
  const motif = typeof params.motif === 'string' && params.motif ? params.motif : 'Collecte solidaire';
  const objectif = useMemo(() => {
    const n = parseInt(String(params.objectif ?? ''), 10);
    return Number.isFinite(n) && n > 0 ? n : null;
  }, [params.objectif]);

  const shareLink = collectId ? buildSolidarityCollectLink(collectId) : '';

  const shareCollect = async () => {
    if (!shareLink) return;
    try {
      await Share.share({
        message: `Soutenez « ${motif} » sur COTICI : ${shareLink}`,
        url: shareLink,
        title: `Collecte solidaire — ${motif}`,
      });
    } catch {
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.hero}>
          ...
          <Text style={styles.heroTitle}>Partager la collecte</Text>
        ...
        <View style={styles.motifPill}>
          ...
          <Text style={styles.motifText} numberOfLines={2}>{motif}</Text>
        </View>
        ...
        <View style={styles.qrPanel}>
          ...
          <Text style={styles.qrHint}>
            Scannez ou partagez ce code pour accéder à la collecte.
          </Text>
        </View>
        ...
        <AnimatedPressable
          ...
          disabled={!collectId}
        >
          <Feather name="eye" size={18} color={Colors.brand} />
          <Text style={styles.viewButtonText}>Voir la collecte</Text>
        </AnimatedPressable>

        <Text style={styles.footerNote}>
          Le numéro du bénéficiaire reste confidentiel. Seuls le motif et la progression sont visibles.
        </Text>
...
const styles = StyleSheet.create({
  ...
  motifText: { fontFamily: Fonts.outfit.medium, fontSize: 14, color: Colors.brand, flex: 1 },
  objectifLabel: {
    fontFamily: Fonts.outfit.regular,
    fontSize: 14,
    color: Colors.gray[600],
  ...
  viewButtonText: { fontFamily: Fonts.outfit.medium, fontSize: 16, color: Colors.brand },
  footerNote: {
    fontFamily: Fonts.outfit.regular,
    fontSize: 12,
    color: Colors.gray[500],
