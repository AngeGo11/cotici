...
import { createCagnotte } from '@/shared/api/cagnotteApi';
...
export default function CreateAssociationFundScreen() {
  ...
  const handleCreate = () => {
    const nom = fundName.trim();
    const descriptionProjet = description.trim();
    const objectif = parsePositiveInt(targetAmount);

    if (!nom) {
      Alert.alert('Champ requis', 'Indiquez un nom pour la cagnotte.');
      return;
    }
    if (!descriptionProjet) {
      Alert.alert('Champ requis', 'Décrivez le projet de la cagnotte.');
      return;
    }
    if (objectif === null) {
    ...
    void (async () => {
      setSubmitting(true);
      const result = await createCagnotte({
        nom_cagnotte: nom,
      ...
      router.replace({
        ...
        params: {
          ...
          nom: result.data.nom_cagnotte,
          motif: result.data.description,
          objectif: String(result.data.objectif_collecte),
        },
      });
    })();
  };
            ...
            <View style={styles.previewRow}>
              <View>
                ...
                <Text style={styles.previewValue}>
                  {(parsePositiveInt(targetAmount) ?? 0).toLocaleString('fr-FR')} F
                </Text>
              </View>
