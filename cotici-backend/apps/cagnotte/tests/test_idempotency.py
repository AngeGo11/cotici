...
cagnotte (idempotency_key optionnelle)."""
...
class CotiserCagnotteIdempotencyTests(APITestCase):
    def setUp(self):
        self.organizer = _create_user("cagnotte_organizer_idem", "22501020311")
        self.contributor = _create_user("cagnotte_contributor_idem", "22509080711")
        self.client.force_authenticate(user=self.contributor)
        Wallet.objects.create(user=self.contributor, solde_courant=Decimal("10000"))

        self.cagnotte = Cagnotte.objects.create(
            hote=self.organizer,
            nom_cagnotte="22501020311",
            description="Projet idempotence",
...
class ConcurrentCotiserCagnotteIdempotencyRaceTests(TransactionTestCase):
    """Deux requêtes de cotisation cagnotte réellement simultanées, portant la
    même clé d'idempotence : une seule doit débiter le wallet et créer la
    Transaction, l'autre doit rejouer la réponse existante."""

    reset_sequences = False

    def setUp(self):
        self.organizer = _create_user("cagnotte_race_organizer", "22501020312")
        self.contributor = _create_user("cagnotte_race_contributor", "22509080712")
        Wallet.objects.create(user=self.contributor, solde_courant=Decimal("10000"))
        self.cagnotte = Cagnotte.objects.create(
            hote=self.organizer,
            nom_cagnotte="22501020312",
            description="Projet course concurrente",
            objectif_cotisation=50000,
            objectif_atteint=False,
            qr_code="x",
        )
